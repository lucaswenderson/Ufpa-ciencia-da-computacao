...
DEFAULT_N_WEEKS = 48  # simula por um ano
NUMBER_OF_ICU_BEDS = 9860  # Número de leitos na França
NUMBER_OF_INITIAL_PATIENTS = 300  # Pacientes iniciais

# Argumentos relacionados ao cenário de simulação 1: 
# Primiero lockdown mais cedo (e mais lockdowns ao longo do ano)
SCENARIO_1_N_BEDS = 5054
# pocentagem de ocupação limite para decretar um lockdown
SCENARIO_1_OCUPATION_TO_LOCKDOWN = 30.0
SCENARIO_1_LOCKDOWN = list(range(7, 21))  # lockdown vai da 2° a 10° semana de simulação

# arguementos relacionados ao cenário de simulação 2: lockdown de dezembro extendido
SCENARIO_2_N_WEEKS = 10
SCENARIO_2_LOCKDOWN = list(range(1, 5))  # lockdown vai da 1° a 4° semana de simulação
SCENARIO_2_PACIENTS = 2896  # Número de leitos ocupados em 14 de dezembro de 2020
...
class ICUMonitor:
        ...
        while True:
            if pdf == "exponential":
                ...
                transit = density_function(
                    EVENTS_STATISTICS[kind]["mean"], size=n_weeks
                )
    ...
    def record_ocupation_percentage(self, week):
        ...
        print(
            f"Semana {week}: {self.total_beds} leitos: {current_held_beds}/{self.beds.level} (ocupados/livres). {round(ocupation_percentage)}% ocupado"
        )
...
def manage_lockdown(
    ...
    ended = False
    if week > lockdown_end_week + 5:
        lockdown_interval = []
        ended = True

    recent_start = lockdown_start_week <= week <= lockdown_start_week + 2
    recent_end = lockdown_end_week <= week <= lockdown_end_week + 5
    ...
    if recent_start:
        ...
        # o período logo após o término do lockdown é marcado por um contágio brando
        transmissibility = 0.75
    elif ended:
...
def run_icu_bed_monitor(env: simpy.Environment, monitor: ICUMonitor, lockdown_interval: List[int]):
    ...
    for week in range(1, monitor.n_weeks + 1):
        ...
        transmissibility, lockdown_interval = manage_lockdown(
            monitor, week, lockdown_interval, transmissibility
        )

        admissions = int(monitor.admissions[week - 1] * transmissibility)

        past_held_beds = monitor.total_beds - monitor.beds.level
        real_discharges = int(monitor.discharges[week - 1])
...
parser = argparse.ArgumentParser(
    description="Simulador de ocupação de leitos de UTI para pacientes com COVID-19."
)
...
def plot_results():
    """ desenha gráficos na linha de comando com alguns resultado relevantes da simulação """
...
def simulate():
    ...
    monitor = ICUMonitor(
        env,
        n_weeks=SCENARIO_2_N_WEEKS if args.scenario_2 else args.n_weeks,
        n_patients=SCENARIO_2_PACIENTS if args.scenario_2 else args.n_init_patients,
        n_beds=SCENARIO_1_N_BEDS if args.scenario_1 else args.n_beds,
        ocupation_to_lockdown=SCENARIO_1_OCUPATION_TO_LOCKDOWN
        if args.scenario_1
        else float("inf"),
        adm_pdf="normal",
        disc_pdf="normal",
    ...
    env.process(
        run_icu_bed_monitor(
            env=env,
            monitor=monitor,
            lockdown_interval=SCENARIO_1_LOCKDOWN
            if args.scenario_1
            else SCENARIO_2_LOCKDOWN
            if args.scenario_2
            else [],
        )
    )
    ...
    numpy.savetxt(
        "results.csv",
        numpy.round(ocupation_percentage_history),
        delimiter=",",
        fmt="%1.1f",
    )
